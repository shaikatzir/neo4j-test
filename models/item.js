...
Item.prototype.getItemProperties = function (callback) {
   // query all items and whether have connection or not:
    var query = [
        ...
        'RETURN other, COUNT(rel) , item.pic as pic, item.detailed_desc as descripstion',  // COUNT(rel) is a hack for 1 or 0
        'ORDER BY other.name'
    ].join('\n')
    ...
    db.query(query, params, function (err, results) {
       ...
       callback(null, properties, others, results[0]['pic'], results[0]['descripstion']);
    });
};

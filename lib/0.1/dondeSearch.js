 ...
 */
numRetItems = 16
misc_no_prefix = Questions.MISC_NO_PREFIX
exports.dynamicSearch = function (params, db, callback) {
   ...
   Search.get_create_search_node(node_name,"search_node", function (err, node) {
	    ...
	    cat = orgCat = params.name
   	    key = params.value
	    arr =  Questions.QUESTIONS_VAL[cat][key]

		//move to next question (unless we are in Misc, then we have to go through all values)
	    var index = Questions.QUESTIONS_ORDER.indexOf(cat);
	    if (orgCat != "Misc")
	    	index++;
	    ...
	    Search.search_create_rel(node, cat, arr,function(err) {
	    	...
	    	Search.search_items(node, function(err,items,count) {
    			...
    			//not last question (and couldn't find a match) - send the next questions and values
					...
					is_misc_question = false;
					//if category is misc, need to split questions to "yes/no" questions for each misc
					if( q_cat == 'Misc' )    {
						//get cuurent key in 'misc'
						
						misc_key = params.value.replace(misc_no_prefix,"");
						
						console.log(misc_key);
						
						//if its the first time we ask 'misc', 'misc_index' will be 0 (-1 + 0)
						var misc_index = Questions.MISC_ORDER.indexOf(misc_key) + 1;
						//check if it was last misc question
						if (misc_index==Questions.MISC_ORDER.length){
							is_misc_question = false;
							q_cat =  Questions.QUESTIONS_ORDER[index+1];
						}
						else{
							is_misc_question = true;							
						}
					}
					ans['nextFeature'] = q_cat; 
					if (is_misc_question){
						misc_key = Questions.MISC_ORDER[misc_index];
						
						ans['values'] = [{"value": misc_key, "img":""},
										{"value": misc_no_prefix+misc_key, "img": ""}] //"key" and "no_key"
						ans['question'] = misc_key					
					}
					else{
